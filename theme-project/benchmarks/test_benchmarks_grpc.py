...
def save_results(timings: list[float], mean: float, variance: float, std: float):
    ...
    # CSV
    with open("results/grpc/timings.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    # Статистика
    with open("results/grpc/stats.txt", "w") as f:
    ...
    plt.tight_layout()
    plt.savefig("results/grpc/plot.png", dpi=150)
    plt.close()

    print("\nРезультаты сохранены в results/grpc/")
